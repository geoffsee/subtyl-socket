...
import { type EncryptionKeys } from '../BaseEncryptionPlugin.ts';
...
console.log(
  '✅ Message processing:',
  JSON.stringify(testMessage) === JSON.stringify(processedIn) ? 'PASSED' : 'FAILED',
);
...
console.log(
  'Unencrypted message processing:',
  JSON.stringify(testMessage) === JSON.stringify(unencryptedIn) ? 'PASSED' : 'FAILED',
);
...
console.log(
  ...
  plugin.getAlgorithmName() === 'aes-256-gcm' ? 'PASSED' : 'FAILED',
);
