    ...
    try {
      ...
      if (msg.type === 'handshake-response') {
        ...
        if (complete.confirmed) {
          ...
          console.log(
            '🔑 Provider: Derived encryption key:',
            providerKeys?.encryptionKey.toString('hex').substring(0, 16) + '...',
          );

          // Initialize encryption plugin with derived keys
          providerEncryption = new MessageEncryptionPlugin(providerKeys!);
          console.log(
            '🔐 Provider: Encryption plugin initialized with algorithm:',
            providerEncryption.getAlgorithmName(),
          );

          // Send encrypted greeting using the plugin
          const secret =
            'Greetings! This message encrypted with MessageEncryptionPlugin. -B. Franklin';
    ...
    try {
      ...
      if (msg.type === 'handshake-init') {
        ...
        if (confirmation.confirmed) {
          ...
          console.log(
            '🔑 Consumer: Derived encryption key:',
            consumerKeys?.encryptionKey.toString('hex').substring(0, 16) + '...',
          );

          // Initialize encryption plugin with derived keys
          consumerEncryption = new MessageEncryptionPlugin(consumerKeys!);
          console.log(
            '🔐 Consumer: Encryption plugin initialized with algorithm:',
            consumerEncryption.getAlgorithmName(),
          );
        ...
        const reply =
          'Your secure message received with gratitude. The MessageEncryptionPlugin experiment succeeds! -A Fellow Natural Philosopher';
